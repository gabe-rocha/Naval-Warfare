...
#pragma warning restore 414
        ...
        GameObject _drawShoreWaveGenQuad;
        ...
        void InitShoreWaveGenQuad()
        {
            Renderer qr;

            if (_drawShoreWaveGenQuad == null)
            {
                _drawShoreWaveGenQuad = GameObject.CreatePrimitive(PrimitiveType.Quad);
                _drawShoreWaveGenQuad.hideFlags = HideFlags.DontSave;
#if UNITY_EDITOR
                DestroyImmediate(_drawShoreWaveGenQuad.GetComponent<Collider>());
#else
                Destroy(_drawShoreWaveGenQuad.GetComponent<Collider>());
#endif
                _drawShoreWaveGenQuad.name = "ShoreWaveGen_" + gameObject.name + "_NOSAVE";
                _drawShoreWaveGenQuad.transform.SetParent(transform, false);
                _drawShoreWaveGenQuad.transform.localEulerAngles = 90f * Vector3.right;
                RegisterShoreWaveInput shoreInput = _drawShoreWaveGenQuad.AddComponent<RegisterShoreWaveInput>();
                //shoreInput._depthTexture = _cacheTexture;
                qr = _drawShoreWaveGenQuad.GetComponent<Renderer>();
                qr.sharedMaterial.mainTexture = _cacheTexture;
            }
            else
            {
        ...
        public void BeginCameraRendering(ScriptableRenderContext context, Camera camera)
        {
            ...
            Material copyDepthMaterial = new Material(Shader.Find("BlueOcean/Copy Depth Buffer Into Cache"));
