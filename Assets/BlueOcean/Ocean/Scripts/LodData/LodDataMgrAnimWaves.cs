...
namespace Ocean
{
    ...
    public class LodDataMgrAnimWaves : LodDataMgr
    {
        ...
        protected override void InitData()
        {
            ...
            var combineShader = Shader.Find("Hidden/BlueOcean/Simulation/Combine Animated Wave LODs");
        ...
        public class FilterFFT : IDrawFilter
        {
            public int _lodIdx;
            public int _lodCount;
            ...
            public float Filter(ILodDataInput data, out int isTransition)
            {
                ...
                if (_lodIdx >= _lodCount)
                {
                    return 0;
                }

                if (data.BatchIdx != _lodIdx)
                {
                    return 0;
                }
        ...
        FilterFFT _filterFFT = new FilterFFT();

        public class FilterShore : IDrawFilter
        {
        ...
        FilterShore _filterShore = new FilterShore();
        ...
        public override void BuildCommandBuffer(OceanRenderer ocean, CommandBuffer buf)
        {
            ...
            for (int lodIdx = lodCount - 1; lodIdx >= 0; lodIdx--)
            {
                ...
                // draw any data with lod preference
                _filterFFT._lodIdx = lodIdx;
                SubmitDrawsFiltered(lodIdx, buf, _filterFFT);
            }
            ...
            for (int lodIdx = lodCount - 1; lodIdx >= 0; lodIdx--)
            {
                ...
                // draw any data with lod preference
                SubmitDrawsFiltered(lodIdx, buf, _filterShore);
            }
        }
        ...
        public void BindLodData(IPropertyWrapper properties)
        {
            BindData(properties, null, true, ref OceanRenderer.Instance._lodTransform._renderData, false);
        }
