#ifndef BLUE_OCEAN_OCEAN_NORMAL_MAPPING_INCLUDED
#define BLUE_OCEAN_OCEAN_NORMAL_MAPPING_INCLUDED
...
half2 SampleNormalMaps(float2 worldXZUndisplaced, float lodAlpha)
{
	...
	half2 norm =
		UnpackNormal(tex2D(_Normals, (v0*_BlueOceanTime*spdmulL + worldXZUndisplaced) / nstretch)).xy +
		UnpackNormal(tex2D(_Normals, (v1*_BlueOceanTime*spdmulL + worldXZUndisplaced) / nstretch)).xy;
	...
	if (nblend > 0.001)
	{
		...
		norm = lerp(norm,
			UnpackNormal(tex2D(_Normals, (v0*_BlueOceanTime*spdmulH + worldXZUndisplaced) / nstretch)).xy +
			UnpackNormal(tex2D(_Normals, (v1*_BlueOceanTime*spdmulH + worldXZUndisplaced) / nstretch)).xy,
			nblend);
	}
...
#endif // BLUE_OCEAN_OCEAN_NORMAL_MAPPING_INCLUDED
