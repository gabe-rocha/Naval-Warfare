#ifndef BLUE_OCEAN_OCEAN_FOAM_INCLUDED
#define BLUE_OCEAN_OCEAN_FOAM_INCLUDED
...
half WhiteFoamTexture(half i_foam, float2 i_worldXZUndisplaced, half lodVal)
{
    ...
    half ft0 = lerp(
        tex2D(_FoamTexture, (1.25*i_worldXZUndisplaced + _BlueOceanTime * v0 * 2) / (4.*_LD_Params[_LD_SliceIndex].x*_FoamScale)).r,
		tex2Dlod(_FoamTexture, float4((1.25*i_worldXZUndisplaced + _BlueOceanTime * v1 * 2) / (4.*_LD_Params[_LD_SliceIndex].x*_FoamScale), 0, 3)).r,
        0.5);

	half ft1 = lerp(
		tex2D(_FoamTexture, (1.25*i_worldXZUndisplaced + _BlueOceanTime * v0 * 2) / (4.*_LD_Params[_LD_SliceIndex + 1].x*_FoamScale)).r,
		tex2Dlod(_FoamTexture, float4((1.25*i_worldXZUndisplaced + _BlueOceanTime * v1 * 2) / (4.*_LD_Params[_LD_SliceIndex + 1].x*_FoamScale), 0, 3)).r,
		0.5);

	half ft = lerp(ft0, ft1, lodVal);
...
half BubbleFoamTexture(float2 i_worldXZ, float2 i_worldXZUndisplaced, half3 i_n, half3 i_view, half lodVal)
{
	...
	float2 foamUVBubbles = (lerp(i_worldXZUndisplaced, i_worldXZ, 0.7) + 0.5 * _BlueOceanTime * windDir) / _FoamScale + 0.125 * i_n.xz;
	float2 parallaxOffset = -_FoamBubbleParallax * i_view.xz / dot(i_n, i_view);
	half ft = lerp(
		tex2Dlod(_BubbleTexture, float4((0.74 * foamUVBubbles + parallaxOffset) / (4.0*_LD_Params[_LD_SliceIndex].x), 0., 3.)).r,
		tex2Dlod(_BubbleTexture, float4((0.74 * foamUVBubbles + parallaxOffset) / (4.0*_LD_Params[_LD_SliceIndex + 1].x), 0., 3.)).r,
		lodVal);
...
void ComputeFoamWithFlow(half2 flow, half i_foam, float2 i_worldXZUndisplaced, float2 i_worldXZ, half3 i_n, float i_pixelZ, float i_sceneZ, half3 i_view, half3 i_lightDir, half3 i_lightCol, half i_shadow, half lodVal, out half3 o_bubbleCol, out half4 o_whiteFoamCol)
{
	...
	float sample1_offset = fmod(_BlueOceanTime, period);
	float sample1_weight = sample1_offset / half_period;
	if (sample1_weight > 1.0) sample1_weight = 2.0 - sample1_weight;
	float sample2_offset = fmod(_BlueOceanTime + half_period, period);
...
#endif // BLUE_OCEAN_OCEAN_FOAM_INCLUDED
