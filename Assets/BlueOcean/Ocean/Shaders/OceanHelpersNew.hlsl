...
#ifndef BLUE_OCEAN_OCEAN_HELPERS_H
#define BLUE_OCEAN_OCEAN_HELPERS_H
...
//void SampleDisplacementsNormalsFoam(in Texture2DArray i_dispSampler, in float3 i_uv_slice, in float i_wt, in float i_invRes, in float i_texelSize, inout float3 io_worldPos, inout half2 io_nxz, inout half io_sss, inout half io_foam)
//{
//	const half4 data = i_dispSampler.SampleLevel(LODData_linear_clamp_sampler, i_uv_slice, 0.0);
//	io_sss += i_wt * data.a;
//	const half3 disp = data.xyz;
...
//		io_foam += 3.0 * 1.59 */*_WaveFoamStrength **/ saturate(0.714/*_WaveFoamCoverage*/ - det);
//	}
//	io_nxz += i_wt * n.xz;
//}
...
//void SampleFoamDirect(in Texture2DArray i_dispSampler, in float3 i_uv_slice, in float i_wt, in float i_invRes, in float i_choppiness, inout half io_foam)
//{
//	float2 dd = float2(i_invRes, 0.0);
//	half3 disp_l = i_dispSampler.SampleLevel(LODData_linear_clamp_sampler, i_uv_slice + float3(-dd, 0.0), 0).xyz;
//	half3 disp_r = i_dispSampler.SampleLevel(LODData_linear_clamp_sampler, i_uv_slice + float3(dd, 0.0), 0).xyz;
//	half3 disp_t = i_dispSampler.SampleLevel(LODData_linear_clamp_sampler, i_uv_slice + float3(-dd, 0.0), 0).xyz;
//	half3 disp_b = i_dispSampler.SampleLevel(LODData_linear_clamp_sampler, i_uv_slice + float3(dd, 0.0), 0).xyz;
//	float3 diff_x = disp_r - disp_l;
//	float3 diff_z = disp_b - disp_t;
//	float2 Dx = diff_x.xz * i_choppiness;
//	float2 Dy = diff_z.xz * i_choppiness;
...
{
	io_oceanDepth += i_wt * (i_oceanDepthSampler.SampleLevel(LODData_linear_clamp_sampler, i_uv_slice, 0.0).x - BLUE_OCEAN_OCEAN_DEPTH_BASELINE);
}
