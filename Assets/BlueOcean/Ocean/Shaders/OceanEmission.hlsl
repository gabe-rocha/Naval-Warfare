#ifndef BLUE_OCEAN_OCEAN_EMISSION_INCLUDED
#define BLUE_OCEAN_OCEAN_EMISSION_INCLUDED
	...
	in const bool i_underwater, in const bool i_outscatterLight, const half3 lightColour, half sss)
{
	...
	if (i_underwater)
	{
		...
		depth = BLUE_OCEAN_OCEAN_DEPTH_BASELINE;
...
void ApplyCaustics(in const float3 scenePos, in const half3 i_lightCol, in const half3 i_lightDir, in const float i_sceneZ, in sampler2D i_normals, in const bool i_underwater, inout half3 io_sceneColour)
{
	...
	float4 cuv1 = float4((surfacePosXZ / _CausticsTextureScale + float2(0.044*_BlueOceanTime + 17.16, -0.169*_BlueOceanTime)), 0., mipLod);
	float4 cuv2 = float4((1.37*surfacePosXZ / _CausticsTextureScale + float2(0.248*_BlueOceanTime, 0.117*_BlueOceanTime)), 0., mipLod);
	...
	in const half3 i_bubbleCol, in sampler2D i_normals, in const bool i_underwater, in const half3 i_scatterCol)
{
	...
	if (!i_underwater)
	{
		...
		const float sceneZRefractDevice = SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, i_uvDepth + refractOffset).x;
...
#endif // BLUE_OCEAN_OCEAN_EMISSION_INCLUDED
