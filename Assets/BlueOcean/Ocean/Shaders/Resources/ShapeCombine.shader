...
Shader "Hidden/BlueOcean/Simulation/Combine Animated Wave LODs"
{
	SubShader
	{
		// No culling or depth
		...
		Pass
		{
			...
			#pragma multi_compile __ BLUE_OCEAN_DYNAMIC_WAVE_SIM_ON_INTERNAL
			#pragma multi_compile __ BLUE_OCEAN_FLOW_ON_INTERNAL
			...
			void Flow(out float2 offsets, out float2 weights)
			{
				...
				offsets = fmod(float2(_BlueOceanTime, _BlueOceanTime + half_period), period);
			...
			half4 Frag(Varyings input) : SV_Target
			{
...
#if BLUE_OCEAN_FLOW_ON_INTERNAL
				half2 flow = 0.0;
				...
				result += data.xyz;
				sss = data.w;
#endif // BLUE_OCEAN_FLOW_ON_INTERNAL
...
#if BLUE_OCEAN_DYNAMIC_WAVE_SIM_ON_INTERNAL
				{
...
#endif // BLUE_OCEAN_DYNAMIC_WAVE_SIM_ON_INTERNAL
